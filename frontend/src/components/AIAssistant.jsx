...
const AIAssistant = () => {
  ...
  const bubbleUser = 'bg-[#0E4B49] text-white rounded-br-md';
    ...
    <>
      {!open && (
        <button
          ...
          className="fixed bottom-6 right-6 z-50 w-14 h-14 bg-gradient-to-br from-[#0E4B49] to-[#1E3A8A] hover:scale-105 text-white rounded-full shadow-xl flex items-center justify-center transition-all"
      ...
      {open && (
        <div className="fixed bottom-6 right-6 z-50 w-[400px] max-w-[calc(100vw-2rem)] h-[600px] max-h-[calc(100vh-3rem)] bg-white rounded-3xl shadow-2xl border border-gray-200 flex flex-col overflow-hidden">
          <div className="flex items-center justify-between p-4 bg-gradient-to-r from-[#0E4B49] to-[#1E3A8A] text-white flex-shrink-0">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-[11px] text-emerald-100">Asistenti yt inteligjent</p>
              </div>
            </div>
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3 bg-gray-50">
            {messages.length === 0 && (
              <div className="text-center py-4">
                <div className="w-14 h-14 bg-gradient-to-br from-[#0E4B49] to-[#1E3A8A] rounded-2xl mx-auto flex items-center justify-center mb-3 shadow-lg">
                  ...
                  <div className="mt-5 space-y-2">
                    {suggestions.map((s, i) => (
                      <button
                        ...
                        className="w-full text-left text-sm px-3 py-2.5 bg-white border border-gray-200 hover:border-[#0E4B49] hover:bg-emerald-50 rounded-xl transition-all"
                      >
                        {s}
                      </button>
                    ))}
            ...
            {messages.map((m, i) => (
              <div key={i} className={'flex gap-2 ' + (m.role === 'user' ? 'justify-end' : 'justify-start')}>
                {m.role === 'assistant' && (
                  <div className="w-8 h-8 bg-gradient-to-br from-[#0E4B49] to-[#1E3A8A] rounded-full flex-shrink-0 flex items-center justify-center">
          ...
          <div className="p-3 bg-white border-t border-gray-200 flex-shrink-0">
            <div className="flex gap-2">
              <input
                ...
                disabled={loading}
                className="flex-1 px-4 h-11 bg-gray-100 rounded-xl text-sm border-0 focus:ring-2 focus:ring-[#0E4B49] focus:bg-white outline-none transition-all disabled:opacity-50"
              />
              <button
                onClick={() => sendMessage()}
                disabled={loading || !input.trim()}
                className="w-11 h-11 bg-[#0E4B49] hover:bg-[#0A3634] disabled:bg-gray-300 text-white rounded-xl flex items-center justify-center transition-colors flex-shrink-0"
              >
